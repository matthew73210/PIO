...
bool RTSPServer::handleRTSPRequest(RTSP_Session& session) {
  ...
  if (!buffer) {
    buffer = (char *)heap_caps_malloc(RTSP_BUFFER_SIZE, MALLOC_CAP_8BIT);
    if (buffer) {
      RTSP_LOGW(LOG_TAG, "ps_malloc failed, using internal RAM");
    } else {
      RTSP_LOGE(LOG_TAG, "Failed to allocate buffer for RTSP request");
      return false;
    }
  }
